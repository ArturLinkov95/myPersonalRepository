...
function changeStatus(name, priority) {
    ...
    if (toDoListIndex !== -1 && list[toDoListIndex].status === "To Do") {
        list[toDoListIndex].status = "Done", list[toDoListIndex].priority = priority;
    }
    else {
...
form.addEventListener('submit', function (event) {
    ...
    function addTask(name) {
        ...
        if (input.value.length < 3 || input.value.length > 30) {
            ...
            const tasks = new Task(name, textPriority);

            list.push({ name: tasks.name, status: tasks.status, priority: tasks.priority}),
                showToDoList();
