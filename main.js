...
let cityCurrent = loadCurrentCities();

console.log(cityCurrent);

let cityFavorite = loadFavoriteCities();

function drawLocalFavorite() {
    for (let i = 0; i < cityFavorite.length; i++) {
        const city = cityFavorite[i];
...
function addCity(cityName) {
    ...
    const indexCityFavorite = cityFavorite.indexOf(lowerCityReplace);

    if (indexCityFavorite != -1) {
        try {
        ...
        cityFavorite.push(lowerCityReplace);
...
function deleteCity(cityName) {
    const indexCity = cityFavorite.indexOf(cityName);
    if (indexCity != -1) {
        cityFavorite.splice(indexCity, 1);
